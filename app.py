...
from utils import (
    ...
    create_event, get_event_by_id, get_events_for_user, get_events_created_by_user,
    add_event_participant, remove_event_participant, update_participant_status,
    get_event_participants, get_upcoming_events
)
import datetime
...
def chat_history(username):
    ...
    messages = get_messages(chat_id, offset=(page-1)*per_page, limit=per_page)
...
def group_history(group_name):
    ...
    messages = get_group_messages(group['id'], offset=(page-1)*per_page, limit=per_page)
    try:
...
def channel_history(channel_name):
    ...
    messages = get_channel_messages(channel['id'], offset=(page-1)*per_page, limit=per_page)
...
def feed_data():
    ...
    try:
        feed = get_feed(user_id, offset=(page-1)*per_page, limit=per_page)
...
def advanced_search():
    """
    Расширенный поиск по содержимому сообщений и постов
    """
    print(f"advanced_search called, session user_id: {session.get('user_id')}")
    if 'user_id' not in session:
        print("No session in advanced_search")
        return jsonify({'error': 'Not authorized'}), 401
    
    query = request.args.get('q', '').strip()
    search_type = request.args.get('type', 'all')  # all, messages, posts
    case_sensitive = request.args.get('case_sensitive', 'false').lower() == 'true'
    limit = request.args.get('limit', 50, type=int)
    offset = request.args.get('offset', 0, type=int)
    
    print(f"Advanced search - Query: '{query}', Type: {search_type}, Case sensitive: {case_sensitive}")
    ...
    try:
        ...
        if search_type == 'messages':
            # Только поиск по сообщениям
            results = search_messages_global(query, user_id, case_sensitive, limit, offset)
            return jsonify({
                'messages': results,
                'posts': [],
            ...
            # Только поиск по постам
            results = search_posts_global(query, user_id, case_sensitive, limit, offset)
            return jsonify({
                'messages': [],
                'posts': results,
                'total_messages': 0,
            ...
            results = search_all_content(query, user_id, case_sensitive, limit, offset)
...
@app.route('/search_in_chat/<chat_partner>')
def search_in_chat(chat_partner):
    """
    Локальный поиск по сообщениям в конкретном чате
    """
    print(f"search_in_chat called for partner: {chat_partner}, session user_id: {session.get('user_id')}")
    if 'user_id' not in session:
        print("No session in search_in_chat")
        return jsonify({'error': 'Not authorized'}), 401
    ...
    try:
        user_id = session['user_id']
        results = search_messages_in_chat(chat_partner, query, user_id, case_sensitive, limit, offset)
        return jsonify({
            'messages': results,
            'query': query,
        ...
        print(f"Error in search_in_chat: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500


@app.route('/search_in_group/<group_name>')
def search_in_group(group_name):
    """
    Локальный поиск по сообщениям в конкретной группе
    """
    print(f"search_in_group called for group: {group_name}, session user_id: {session.get('user_id')}")
    if 'user_id' not in session:
        print("No session in search_in_group")
        return jsonify({'error': 'Not authorized'}), 401
    ...
    try:
        user_id = session['user_id']
        results = search_messages_in_group(group_name, query, user_id, case_sensitive, limit, offset)
        return jsonify({
            'messages': results,
            'query': query,
        ...
        print(f"Error in search_in_group: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500


@app.route('/search_in_channel/<channel_name>')
def search_in_channel(channel_name):
    """
    Локальный поиск по сообщениям в конкретном канале
    """
    print(f"search_in_channel called for channel: {channel_name}, session user_id: {session.get('user_id')}")
    if 'user_id' not in session:
        print("No session in search_in_channel")
        return jsonify({'error': 'Not authorized'}), 401
    ...
    try:
        user_id = session['user_id']
        results = search_messages_in_channel(channel_name, query, user_id, case_sensitive, limit, offset)
        return jsonify({
            'messages': results,
            'query': query,
        ...
        print(f"Error in search_in_channel: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
# === Обработчики Socket.IO для мероприятий ===
@socketio.on('invite_to_event')
def handle_event_invite(data):
    event_id = data['event_id']
    username = data['username']
    inviter_username = session.get('username')
    
    if not inviter_username:
        emit('error', {'msg': 'Пользователь не авторизован'})
        return
    
    # Проверить, является ли пользователь организатором мероприятия
    event = get_event_by_id(event_id)
    if not event:
        emit('error', {'msg': 'Мероприятие не найдено'})
        return
    
    inviter_user = get_user_by_username(inviter_username)
    if event['creator_id'] != inviter_user['id']:
        emit('error', {'msg': 'У вас нет прав приглашать на это мероприятие'})
        return
    
    # Пригласить пользователя
    invitee_user = get_user_by_username(username)
    if not invitee_user:
        emit('error', {'msg': 'Пользователь не найден'})
        return
    
    add_event_participant(event_id, invitee_user['id'], 'invited')
    
    # Отправить уведомление приглашенному пользователю
    invitee_sid = user_sids.get(username)
    if invitee_sid:
        emit('event_invitation', {
            'event_id': event_id,
            'event_title': event['title'],
            'inviter': inviter_username
        }, room=invitee_sid)
    
    emit('event_invite_sent', {
        'event_id': event_id,
        'username': username
    })
...
def events():
    ...
    created_events = get_events_created_by_user(user_id)
...
def create_event():
    ...
    if request.method == 'POST':
        ...
        event_id = create_event(title, description, event_date, location, session['user_id'])
        
        # Добавить создателя как подтвержденного участника
        add_event_participant(event_id, session['user_id'], 'confirmed')
...
def join_event(event_id):
    ...
    add_event_participant(event_id, user_id, 'confirmed')
...
def leave_event(event_id):
    ...
    # Не позволять покинуть мероприятие, если пользователь является организатором
    if event['creator_id'] == user_id:
        return "Организатор не может покинуть мероприятие", 400
    
    remove_event_participant(event_id, user_id)
...
def update_event_status(event_id):
    ...
    if status not in ['invited', 'confirmed', 'declined']:
        return "Недопустимый статус", 400
    
    update_participant_status(event_id, user_id, status)
...
def invite_to_event(event_id):
    ...
    if event['creator_id'] != session['user_id']:
        return "У вас нет прав приглашать на это мероприятие", 403
    ...
    add_event_participant(event_id, user['id'], 'invited')
