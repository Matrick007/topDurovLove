...
from utils import (
    ...
    update_group_message_read, edit_post, delete_post
)
import datetime
...
def update_profile():
    ...
    with get_db_connection() as conn:
        conn.execute("""
            UPDATE users SET city = ?, bio_short = ?, country = ?, languages = ?, bio_full = ?, hobbies = ?, status = ? WHERE id = ?
        """, (city, bio_short, country, languages, bio_full, hobbies, status, session['user_id']))
        conn.commit()
...
def feed_data():
    if 'user_id' not in session:
        return jsonify({'feed': []})
    feed = get_feed(session['user_id'])
    return jsonify({'feed': [{'id': p['id'], 'username': p['username'], 'content': p['content'], 'image_url': p['image_url'], 'likes_count': p['likes_count'], 'comments_count': p['comments_count'], 'reposts_count': p['reposts_count']} for p in feed]})
